...
int SpotifyArduino::searchForPlaylist(String query, int limit, processPlaylistSearch searchCallback, PlaylistSearchResult results[])
{
    ...
    if (statusCode == 200)
    {
        ...
        if (!error)
        {
            ...
            uint8_t totalResults = doc["playlists"]["items"].size();

            Serial.print("Total Results: ");
            Serial.println(totalResults);

            PlaylistSearchResult searchResult;
            for (int i = 0; i < totalResults; i++)
            {
                //Polling track information
                JsonObject result = doc["playlists"]["items"][i];
                searchResult.playlistUri = result["uri"].as<const char *>();
                searchResult.playlistName = result["name"].as<const char *>();
                searchResult.playlistId = result["id"].as<const char *>();
                searchResult.numTracks = result["tracks"]["total"].as<int>();;
                searchResult.tracksHref = result["tracks"]["href"].as<const char *>();
...
int SpotifyArduino::getPlaylistItems(String query, const char *playlistId, int limit, processSearch searchCallback, SearchResult results[])
{
    ...
    const char *command = ("/v1/playlists/" + (String) playlistId + "/tracks" + query).c_str();
    ...
    if (statusCode == 200)
    {
        ...
        if (!error)
        {
            ...
            uint8_t totalResults = doc["total"].as<int>();
            ...
            {
                //Polling track information
                JsonObject result = doc["items"][i]["track"];
                searchResult.trackUri = result["uri"].as<const char *>();
                searchResult.trackName = result["name"].as<const char *>();
                searchResult.albumUri = result["album"]["uri"].as<const char *>();
                searchResult.albumName = result["album"]["name"].as<const char *>();
