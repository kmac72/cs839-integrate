...
#define SPOTIFY_MAX_NUM_TRACKS 20 // Just for us, we only care about the first 20 tracks
...
typedef bool (*processPlaylistSearch)(PlaylistSearchResult result, int index, int numResults);
...
public:
  ...
  int searchForPlaylist(String query, int limit, processPlaylistSearch searchCallback, PlaylistSearchResult results[]);
  int getPlaylistItems(String query, const char *playlistId, int limit, processSearch searchCallback, SearchResult results[]);
